...
def DataFame_to_pajek(
    ...
    a1 = vinc[[emisor_index, emisor_column]].drop_duplicates(
        emisor_index).sort_values(emisor_index).reset_index(drop=True)
    a1[emisor_column] = a1[emisor_column].str.replace(
        '\n', ' ').str.replace('^([0-0a-zA-Z]+)$', r'\1 ')
    a2 = vinc[[receptor_index, receptor]].drop_duplicates(
        receptor).sort_values(receptor_index).reset_index(drop=True)
